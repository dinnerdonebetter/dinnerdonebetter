...
class TaskListViewModel {
  ...
  func areAllSubtasksCompleted(parentTaskID: String) -> Bool {
    ...
    print("🔍 areAllSubtasksCompleted for task \(parentTaskID): \(allCompleted) (total steps: \(stepStates.count), completed: \(stepStates.values.filter { $0 }.count))")
    return allCompleted
  }
  ...
  private func getStepDataForTask(_ task: Mealplanning_MealPlanTask) -> [StepInfo] {
    ...
    guard let recipe = loadedRecipes[recipeID] else {
      print("⚠️ Task \(task.id): Recipe \(recipeID) not loaded yet. Loaded recipes: \(loadedRecipes.keys.joined(separator: ", "))")
      return []
    }
  ...
  private func createSubtaskFromStep(task: Mealplanning_MealPlanTask, stepInfo: StepInfo) -> Mealplanning_MealPlanTask {
  ...
  func getUnfinishedGroups() -> [TaskGroup] {
    ...
    print("📋 getUnfinishedGroups: \(unfinished.count) groups (parent statuses: \(allGroups.map { $0.parent.status }))")
    return unfinished
  }
  ...
  func getFinishedGroups() -> [TaskGroup] {
    ...
    print("📋 getFinishedGroups: \(finished.count) groups (parent statuses: \(allGroups.map { $0.parent.status }))")
    return finished
  }
  ...
  func toggleTaskStatus(_ task: Mealplanning_MealPlanTask) async {
    ...
    do {
      let newStatus: Mealplanning_MealPlanTaskStatus = task.status == .finished
        ? .unfinished : .finished
