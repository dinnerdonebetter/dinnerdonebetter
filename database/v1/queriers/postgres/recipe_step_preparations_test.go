...
func TestPostgres_buildGetRecipeStepPreparationsQuery(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		expectedQuery := "SELECT recipe_step_preparations.id, recipe_step_preparations.valid_preparation_id, recipe_step_preparations.notes, recipe_step_preparations.created_on, recipe_step_preparations.updated_on, recipe_step_preparations.archived_on, recipe_step_preparations.belongs_to_recipe_step, COUNT(recipe_step_preparations.id) FROM recipe_step_preparations JOIN recipe_steps ON recipe_step_preparations.belongs_to_recipe_step=recipe_steps.id JOIN recipes ON recipe_steps.belongs_to_recipe=recipes.id WHERE recipe_step_preparations.archived_on IS NULL AND recipe_step_preparations.belongs_to_recipe_step = $1 AND recipe_steps.belongs_to_recipe = $2 AND recipe_steps.id = $3 AND recipes.id = $4 AND recipe_step_preparations.created_on > $5 AND recipe_step_preparations.created_on < $6 AND recipe_step_preparations.updated_on > $7 AND recipe_step_preparations.updated_on < $8 GROUP BY recipe_step_preparations.id LIMIT 20 OFFSET 180"
		expectedArgs := []interface{}{
			exampleRecipeStep.ID,
			exampleRecipe.ID,
			exampleRecipeStep.ID,
...
func TestPostgres_GetRecipeStepPreparations(T *testing.T) {
	...
	expectedListQuery := "SELECT recipe_step_preparations.id, recipe_step_preparations.valid_preparation_id, recipe_step_preparations.notes, recipe_step_preparations.created_on, recipe_step_preparations.updated_on, recipe_step_preparations.archived_on, recipe_step_preparations.belongs_to_recipe_step, COUNT(recipe_step_preparations.id) FROM recipe_step_preparations JOIN recipe_steps ON recipe_step_preparations.belongs_to_recipe_step=recipe_steps.id JOIN recipes ON recipe_steps.belongs_to_recipe=recipes.id WHERE recipe_step_preparations.archived_on IS NULL AND recipe_step_preparations.belongs_to_recipe_step = $1 AND recipe_steps.belongs_to_recipe = $2 AND recipe_steps.id = $3 AND recipes.id = $4 GROUP BY recipe_step_preparations.id LIMIT 20"
