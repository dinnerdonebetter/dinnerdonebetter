...
func TestPostgres_buildGetRecipeStepIngredientsQuery(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		expectedQuery := "SELECT recipe_step_ingredients.id, recipe_step_ingredients.valid_ingredient_id, recipe_step_ingredients.ingredient_notes, recipe_step_ingredients.quantity_type, recipe_step_ingredients.quantity_value, recipe_step_ingredients.quantity_notes, recipe_step_ingredients.product_of_recipe_step_id, recipe_step_ingredients.created_on, recipe_step_ingredients.updated_on, recipe_step_ingredients.archived_on, recipe_step_ingredients.belongs_to_recipe_step, COUNT(recipe_step_ingredients.id) FROM recipe_step_ingredients JOIN recipe_steps ON recipe_step_ingredients.belongs_to_recipe_step=recipe_steps.id JOIN recipes ON recipe_steps.belongs_to_recipe=recipes.id WHERE recipe_step_ingredients.archived_on IS NULL AND recipe_step_ingredients.belongs_to_recipe_step = $1 AND recipe_steps.belongs_to_recipe = $2 AND recipe_steps.id = $3 AND recipes.id = $4 AND recipe_step_ingredients.created_on > $5 AND recipe_step_ingredients.created_on < $6 AND recipe_step_ingredients.updated_on > $7 AND recipe_step_ingredients.updated_on < $8 GROUP BY recipe_step_ingredients.id LIMIT 20 OFFSET 180"
		expectedArgs := []interface{}{
			exampleRecipeStep.ID,
			exampleRecipe.ID,
			exampleRecipeStep.ID,
...
func TestPostgres_GetRecipeStepIngredients(T *testing.T) {
	...
	expectedListQuery := "SELECT recipe_step_ingredients.id, recipe_step_ingredients.valid_ingredient_id, recipe_step_ingredients.ingredient_notes, recipe_step_ingredients.quantity_type, recipe_step_ingredients.quantity_value, recipe_step_ingredients.quantity_notes, recipe_step_ingredients.product_of_recipe_step_id, recipe_step_ingredients.created_on, recipe_step_ingredients.updated_on, recipe_step_ingredients.archived_on, recipe_step_ingredients.belongs_to_recipe_step, COUNT(recipe_step_ingredients.id) FROM recipe_step_ingredients JOIN recipe_steps ON recipe_step_ingredients.belongs_to_recipe_step=recipe_steps.id JOIN recipes ON recipe_steps.belongs_to_recipe=recipes.id WHERE recipe_step_ingredients.archived_on IS NULL AND recipe_step_ingredients.belongs_to_recipe_step = $1 AND recipe_steps.belongs_to_recipe = $2 AND recipe_steps.id = $3 AND recipes.id = $4 GROUP BY recipe_step_ingredients.id LIMIT 20"
