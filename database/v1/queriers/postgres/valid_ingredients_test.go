...
func buildMockRowsFromValidIngredient(validIngredients ...*models.ValidIngredient) *sqlmock.Rows {
...
func TestPostgres_GetValidIngredient(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		mockDB.ExpectQuery(formatQueryForSQLMock(expectedQuery)).
			...
			WillReturnRows(buildMockRowsFromValidIngredient(exampleValidIngredient))
...
func TestPostgres_GetValidIngredients(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		mockDB.ExpectQuery(formatQueryForSQLMock(expectedListQuery)).
			WillReturnRows(
				buildMockRowsFromValidIngredient(
...
func TestPostgres_buildArchiveValidIngredientQuery(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		expectedQuery := "UPDATE valid_ingredients SET updated_on = extract(epoch FROM NOW()), archived_on = extract(epoch FROM NOW()) WHERE archived_on IS NULL AND id = $1 RETURNING archived_on"
		expectedArgs := []interface{}{
			exampleValidIngredient.ID,
		}
