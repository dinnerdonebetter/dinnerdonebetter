...
func (s *Sqlite) buildGetRecipeQuery(recipeID, userID uint64) (query string, args []interface{}) {
	var err error
...
// GetRecipe fetches a recipe from the sqlite database
func (s *Sqlite) GetRecipe(ctx context.Context, recipeID, userID uint64) (*models.Recipe, error) {
	query, args := s.buildGetRecipeQuery(recipeID, userID)
...
func (s *Sqlite) buildGetRecipeCountQuery(filter *models.QueryFilter, userID uint64) (query string, args []interface{}) {
	var err error
...
func (s *Sqlite) GetRecipeCount(ctx context.Context, filter *models.QueryFilter, userID uint64) (count uint64, err error) {
	query, args := s.buildGetRecipeCountQuery(filter, userID)
...
func (s *Sqlite) buildGetAllRecipesCountQuery() string {
	allRecipesCountQueryBuilder.Do(func() {
		var err error
		allRecipesCountQuery, _, err = s.sqlBuilder.
			Select(CountQuery).
			From(recipesTableName).
...
func (s *Sqlite) GetAllRecipesCount(ctx context.Context) (count uint64, err error) {
	err = s.db.QueryRowContext(ctx, s.buildGetAllRecipesCountQuery()).Scan(&count)
	return count, err
}
...
func (s *Sqlite) buildGetRecipesQuery(filter *models.QueryFilter, userID uint64) (query string, args []interface{}) {
	var err error
	builder := s.sqlBuilder.
		Select(recipesTableColumns...).
		From(recipesTableName).
...
func (s *Sqlite) GetRecipes(ctx context.Context, filter *models.QueryFilter, userID uint64) (*models.RecipeList, error) {
	query, args := s.buildGetRecipesQuery(filter, userID)
	...
	list, err := scanRecipes(s.logger, rows)
	if err != nil {
		return nil, fmt.Errorf("scanning response from database: %w", err)
	}

	count, err := s.GetRecipeCount(ctx, filter, userID)
	if err != nil {
...
func (s *Sqlite) GetAllRecipesForUser(ctx context.Context, userID uint64) ([]models.Recipe, error) {
	query, args := s.buildGetRecipesQuery(nil, userID)
...
func (s *Sqlite) buildCreateRecipeQuery(input *models.Recipe) (query string, args []interface{}) {
	var err error
	query, args, err = s.sqlBuilder.
...
func (s *Sqlite) buildRecipeCreationTimeQuery(recipeID uint64) (query string, args []interface{}) {
	var err error
...
func (s *Sqlite) CreateRecipe(ctx context.Context, input *models.RecipeCreationInput) (*models.Recipe, error) {
	x := &models.Recipe{
		Name:               input.Name,
		Source:             input.Source,
	...
	query, args := s.buildCreateRecipeQuery(x)
	...
	if idErr == nil {
		...
		query, args := s.buildRecipeCreationTimeQuery(x.ID)
...
func (s *Sqlite) buildUpdateRecipeQuery(input *models.Recipe) (query string, args []interface{}) {
	var err error
	query, args, err = s.sqlBuilder.
...
func (s *Sqlite) UpdateRecipe(ctx context.Context, input *models.Recipe) error {
	query, args := s.buildUpdateRecipeQuery(input)
	_, err := s.db.ExecContext(ctx, query, args...)
	return err
}

// buildArchiveRecipeQuery returns a SQL query which marks a given recipe belonging to a given user as archived.
func (s *Sqlite) buildArchiveRecipeQuery(recipeID, userID uint64) (query string, args []interface{}) {
	var err error
	query, args, err = s.sqlBuilder.
		Update(recipesTableName).
		Set("updated_on", squirrel.Expr(CurrentUnixTimeQuery)).
...
func (s *Sqlite) ArchiveRecipe(ctx context.Context, recipeID, userID uint64) error {
	query, args := s.buildArchiveRecipeQuery(recipeID, userID)
	_, err := s.db.ExecContext(ctx, query, args...)
	return err
}
