...
func TestV1Client_BuildGetRecipeStepInstrumentRequest(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		expectedID := uint64(1)
		actual, err := c.BuildGetRecipeStepInstrumentRequest(ctx, expectedID)
...
func TestV1Client_GetRecipeStepInstrument(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		ts := httptest.NewTLSServer(
			http.HandlerFunc(
				func(res http.ResponseWriter, req *http.Request) {
					...
					assert.Equal(t, req.URL.Path, fmt.Sprintf("/api/v1/recipe_step_instruments/%d", expected.ID), "expected and actual path don't match")
		...
		c := buildTestClient(t, ts)
		actual, err := c.GetRecipeStepInstrument(ctx, expected.ID)
...
func TestV1Client_BuildGetRecipeStepInstrumentsRequest(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		c := buildTestClient(t, ts)
		actual, err := c.BuildGetRecipeStepInstrumentsRequest(ctx, nil)
...
func TestV1Client_GetRecipeStepInstruments(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		ts := httptest.NewTLSServer(
			http.HandlerFunc(
				func(res http.ResponseWriter, req *http.Request) {
					assert.Equal(t, req.URL.Path, "/api/v1/recipe_step_instruments", "expected and actual path don't match")
					assert.Equal(t, req.Method, http.MethodGet)
					require.NoError(t, json.NewEncoder(res).Encode(expected))
				},
			),
		...
		c := buildTestClient(t, ts)
		actual, err := c.GetRecipeStepInstruments(ctx, nil)
...
func TestV1Client_BuildCreateRecipeStepInstrumentRequest(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		c := buildTestClient(t, ts)
		actual, err := c.BuildCreateRecipeStepInstrumentRequest(ctx, exampleInput)
...
func TestV1Client_BuildUpdateRecipeStepInstrumentRequest(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		ts := httptest.NewTLSServer(nil)
		c := buildTestClient(t, ts)
		actual, err := c.BuildUpdateRecipeStepInstrumentRequest(ctx, exampleInput)
		...
		ts := httptest.NewTLSServer(
			http.HandlerFunc(
				func(res http.ResponseWriter, req *http.Request) {
					...
					assert.NoError(t, json.NewEncoder(res).Encode(&models.RecipeStepInstrument{}))
				},
			),
		)

		err := buildTestClient(t, ts).UpdateRecipeStepInstrument(ctx, expected)
		assert.NoError(t, err, "no error should be returned")
	})
}
...
func TestV1Client_BuildArchiveRecipeStepInstrumentRequest(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		expectedID := uint64(1)
		c := buildTestClient(t, ts)
		actual, err := c.BuildArchiveRecipeStepInstrumentRequest(ctx, expectedID)
...
func TestV1Client_ArchiveRecipeStepInstrument(T *testing.T) {
	...
	T.Run("happy path", func(t *testing.T) {
		...
		ts := httptest.NewTLSServer(
			http.HandlerFunc(
				func(res http.ResponseWriter, req *http.Request) {
					assert.Equal(t, req.URL.Path, fmt.Sprintf("/api/v1/recipe_step_instruments/%d", expected), "expected and actual path don't match")
		...
		err := buildTestClient(t, ts).ArchiveRecipeStepInstrument(ctx, expected)
		assert.NoError(t, err, "no error should be returned")
	})
}
