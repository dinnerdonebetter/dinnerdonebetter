...
func (s *service) CreateHandler(res http.ResponseWriter, req *http.Request) {
	...
	input := new(types.RecipeStepCreationInput)
	...
	recipeStep, err := s.recipeStepDataManager.CreateRecipeStep(ctx, input, sessionCtxData.Requester.UserID)
	if err != nil {
	...
	s.encoderDecoder.EncodeResponseWithStatus(ctx, res, recipeStep, http.StatusCreated)
}
...
func (s *service) UpdateHandler(res http.ResponseWriter, req *http.Request) {
	...
	input := new(types.RecipeStepUpdateInput)
	if err = s.encoderDecoder.DecodeRequest(ctx, req, input); err != nil {
	...
	changeReport := recipeStep.Update(input)
	tracing.AttachChangeSummarySpan(span, "recipe_step", changeReport)

	// update recipe step in database.
	if err = s.recipeStepDataManager.UpdateRecipeStep(ctx, recipeStep, sessionCtxData.Requester.UserID, changeReport); err != nil {
...
func (s *service) ArchiveHandler(res http.ResponseWriter, req *http.Request) {
	...
	err = s.recipeStepDataManager.ArchiveRecipeStep(ctx, recipeID, recipeStepID, sessionCtxData.Requester.UserID)
	if errors.Is(err, sql.ErrNoRows) {
		s.encoderDecoder.EncodeNotFoundResponse(ctx, res)
		return
	} else if err != nil {
...
// AuditEntryHandler returns a GET handler that returns all audit log entries related to a recipe step.
func (s *service) AuditEntryHandler(res http.ResponseWriter, req *http.Request) {
	...
	x, err := s.recipeStepDataManager.GetAuditLogEntriesForRecipeStep(ctx, recipeStepID)
	if errors.Is(err, sql.ErrNoRows) {
		s.encoderDecoder.EncodeNotFoundResponse(ctx, res)
		return
	} else if err != nil {
		observability.AcknowledgeError(err, logger, span, "retrieving audit log entries for recipe step")
		s.encoderDecoder.EncodeUnspecifiedInternalServerErrorResponse(ctx, res)
		return
	}
