...
func TestQuerier_GetTotalValidPreparationCount(T *testing.T) {
	...
	T.Run("standard", func(t *testing.T) {
		...
		db.ExpectQuery(formatQueryForSQLMock(getTotalValidPreparationsCountQuery)).
			WithArgs().
			WillReturnRows(newCountDBRowResponse(uint64(123)))

		actual, err := c.GetTotalValidPreparationCount(ctx)
		assert.NoError(t, err)
...
func TestQuerier_CreateValidPreparation(T *testing.T) {
	...
	T.Run("standard", func(t *testing.T) {
		...
		db.ExpectExec(formatQueryForSQLMock(validPreparationCreationQuery)).
			WithArgs(interfaceToDriverValue(args)...).
			WillReturnResult(newArbitraryDatabaseResult(exampleValidPreparation.ID))
