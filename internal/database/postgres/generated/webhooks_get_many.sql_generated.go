...
const GetWebhooks = `-- name: GetWebhooks :exec
SELECT
    webhooks.id,
    webhooks.name,
...
func (q *Queries) GetWebhooks(ctx context.Context, db DBTX, belongsToHousehold string) error {
	_, err := db.ExecContext(ctx, GetWebhooks, belongsToHousehold)
	return err
}
