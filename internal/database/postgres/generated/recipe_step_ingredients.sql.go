...
SELECT EXISTS ( SELECT recipe_step_ingredients.id FROM recipe_step_ingredients JOIN recipe_steps ON recipe_step_ingredients.belongs_to_recipe_step=recipe_steps.id JOIN recipes ON recipe_steps.belongs_to_recipe=recipes.id WHERE recipe_step_ingredients.archived_on IS NULL AND recipe_step_ingredients.belongs_to_recipe_step = $1 AND recipe_step_ingredients.id = $2 AND recipe_steps.archived_on IS NULL AND recipe_steps.belongs_to_recipe = $3 AND recipe_steps.id = $1 AND recipes.archived_on IS NULL AND recipes.id = $3 )
`
...
func (q *Queries) RecipeStepIngredientExists(ctx context.Context, arg *RecipeStepIngredientExistsParams) (bool, error) {
	row := q.db.QueryRowContext(ctx, RecipeStepIngredientExists, arg.RecipeStepID, arg.RecipeStepIngredientID, arg.RecipeID)
	var exists bool
	err := row.Scan(&exists)
	return exists, err
}
