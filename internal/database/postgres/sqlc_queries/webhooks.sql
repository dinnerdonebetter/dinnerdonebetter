...
SET
	archived_at = NOW()
WHERE
	archived_at IS NULL AND id = sqlc.arg(id) AND belongs_to_household = sqlc.arg(household_id);

-- name: CreateWebhook :exec

INSERT
INTO
	webhooks (id, name, content_type, url, method, belongs_to_household)
VALUES
	(sqlc.arg(id), sqlc.arg(name), sqlc.arg(content_type), sqlc.arg(url), sqlc.arg(method), sqlc.arg(belongs_to_household));
...
SELECT
	...
	(
		...
		AS total_count
FROM
	webhooks JOIN webhook_trigger_events ON webhooks.id = webhook_trigger_events.belongs_to_webhook
WHERE
	webhooks.created_at > COALESCE(sqlc.narg(created_before), (SELECT NOW() - '999 years'::INTERVAL))
	AND webhooks.created_at < COALESCE(sqlc.narg(created_after), (SELECT NOW() + '999 years'::INTERVAL))
	AND (
			webhooks.last_updated_at IS NULL
			OR webhooks.last_updated_at > COALESCE(sqlc.narg(updated_before), (SELECT NOW() - '999 years'::INTERVAL))
