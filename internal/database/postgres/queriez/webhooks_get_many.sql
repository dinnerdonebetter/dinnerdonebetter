...
SELECT
    ...
    SELECT COUNT(webhooks.id)
     ...
     WHERE webhooks.archived_at IS NULL
       ...
       AND (webhooks.last_updated_at IS NULL OR webhooks.last_updated_at > COALESCE(sqlc.narg('updated_after'), (SELECT NOW() - interval '999 years')))
       AND (webhooks.last_updated_at IS NULL OR
            webhooks.last_updated_at < COALESCE(sqlc.narg('updated_before'), (SELECT NOW() + interval '999 years')))
    ) as filtered_count,
    (
        SELECT
            COUNT(webhooks.id)
