...
func TestQuerier_SearchForValidVesselsForPreparation(T *testing.T) {
	T.Parallel()
	...
	T.Run("standard", func(t *testing.T) {
		...
		actual, err := c.SearchForValidVesselsForPreparation(ctx, "", exampleQuery)
		assert.NoError(t, err)
		assert.Equal(t, exampleValidVessels.Data, actual)
	...
	T.Run("with invalid valid vessel ID", func(t *testing.T) {
		...
		actual, err := c.SearchForValidVesselsForPreparation(ctx, "", "")
		assert.Error(t, err)
		assert.Nil(t, actual)
	})
