...
//go:embed queries/service_settings/update.sql
var updateServiceSettingQuery string

// UpdateServiceSetting updates a particular service setting.
func (q *Querier) UpdateServiceSetting(ctx context.Context, updated *types.ServiceSetting) error {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
	...
	logger := q.logger.WithValue(keys.ServiceSettingIDKey, updated.ID)
	tracing.AttachServiceSettingIDToSpan(span, updated.ID)
	...
	if err := q.performWriteQuery(ctx, q.db, "service setting update", updateServiceSettingQuery, args); err != nil {
		return observability.PrepareAndLogError(err, logger, span, "updating service setting")
	}

	logger.Info("service setting updated")
