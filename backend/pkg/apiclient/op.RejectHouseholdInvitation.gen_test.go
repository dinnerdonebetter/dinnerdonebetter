...
func TestClient_RejectAccountInvitation(T *testing.T) {
	T.Parallel()

	const expectedPathFormat = "/api/v1/account_invitations/%s/reject"
	...
	T.Run("standard", func(t *testing.T) {
		...
		err := c.RejectAccountInvitation(ctx, accountInvitationID, exampleInput)
	...
	T.Run("with invalid accountInvitation ID", func(t *testing.T) {
		...
		err := c.RejectAccountInvitation(ctx, "", exampleInput)
