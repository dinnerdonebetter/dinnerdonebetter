...
func TestClient_GetMealPlansForAccount(T *testing.T) {
	T.Parallel()
	...
	T.Run("standard", func(t *testing.T) {
		...
		list := []*MealPlan{}
		exampleResponse := &APIResponse[[]*MealPlan]{
			Pagination: fake.BuildFakeForTest[*Pagination](t),
			Data:       list,
		}
		expected := &QueryFilteredResult[MealPlan]{
		...
		actual, err := c.GetMealPlansForAccount(ctx, nil)
