...
func TestClient_GetValidIngredientStates(T *testing.T) {
	T.Parallel()
	...
	T.Run("standard", func(t *testing.T) {
		...
		list := []*ValidIngredientState{}
		exampleResponse := &APIResponse[[]*ValidIngredientState]{
			Pagination: fake.BuildFakeForTest[*Pagination](t),
			Data:       list,
		}
		expected := &QueryFilteredResult[ValidIngredientState]{
		...
		actual, err := c.GetValidIngredientStates(ctx, nil)
