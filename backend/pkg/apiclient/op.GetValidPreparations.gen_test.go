...
func TestClient_GetValidPreparations(T *testing.T) {
	...
	T.Run("standard", func(t *testing.T) {
		...
		list := []*ValidPreparation{}
		exampleResponse := &APIResponse[[]*ValidPreparation]{
			Pagination: fake.BuildFakeForTest[*Pagination](t),
			Data:       list,
		}
		expected := &QueryFilteredResult[ValidPreparation]{
		...
		actual, err := c.GetValidPreparations(ctx, nil)
