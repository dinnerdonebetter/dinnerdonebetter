...
func (s *AdminFrontendServer) UserPage(_ http.ResponseWriter, req *http.Request) (g.Node, error) {
	...
	auditLogSection := components.CardWithHeader(
		...
		nil,
		g.El("div",
			g.Attr("id", "user-audit-log-container"),
			g.Attr("hx-get", fmt.Sprintf("/api/users/%s/audit-log", userID)),
			g.Attr("hx-trigger", "load"),
			g.Attr("hx-swap", "innerHTML"),
			components.LoadingSpinner(&design.StandardPalette),
		),
	...
	subscriptionsSection := components.CardWithHeader(
		...
		nil,
		g.El("div",
			g.Attr("id", "user-subscriptions-container"),
			g.Attr("hx-get", fmt.Sprintf("/api/users/%s/subscriptions", userID)),
			g.Attr("hx-trigger", "load"),
			g.Attr("hx-swap", "innerHTML"),
			components.LoadingSpinner(&design.StandardPalette),
		),
	...
	// Combine form, accounts, subscriptions, and audit log sections
	return page("Users",
		ghtml.Div(
	...
	tablePageResult, err := components.TablePage(&components.TablePageProps[*identitysvc.User]{
		...
		TableOptions: &components.TableOptions[*identitysvc.User]{
			...
			FieldReplacements: map[string]string{
				"email_address_verified_at": "Email Verified At",
			},
			FieldRenderers: map[string]components.FieldRenderer{
				"birthday":                    renderTimestamp,
				"created_at":                  renderTimestamp,
				"two_factor_secret_verified_at": renderTimestamp,
				"last_updated_at":             renderTimestamp,
				"archived_at":                 renderTimestamp,
				"password_last_changed_at":   renderTimestamp,
				"last_accepted_terms_of_service": renderTimestamp,
				"last_accepted_privacy_policy":  renderTimestamp,
			},
