...
func (s *service) CreateValidPreparationInstrumentHandler(res http.ResponseWriter, req *http.Request) {
	...
	validPreparationInstrument, err := s.validEnumerationDataManager.CreateValidPreparationInstrument(ctx, input)
	if err != nil {
	...
	dcm := &types.DataChangeMessage{
		EventType:                  types.ValidPreparationInstrumentCreatedServiceEventType,
...
func (s *service) ReadValidPreparationInstrumentHandler(res http.ResponseWriter, req *http.Request) {
	...
	x, err := s.validEnumerationDataManager.GetValidPreparationInstrument(ctx, validPreparationInstrumentID)
...
func (s *service) ListValidPreparationInstrumentsHandler(res http.ResponseWriter, req *http.Request) {
	...
	if errors.Is(err, sql.ErrNoRows) {
		...
		validPreparationInstruments = &filtering.QueryFilteredResult[types.ValidPreparationInstrument]{Data: []*types.ValidPreparationInstrument{}}
	} else if err != nil {
...
func (s *service) UpdateValidPreparationInstrumentHandler(res http.ResponseWriter, req *http.Request) {
	...
	// fetch valid preparation instrument from database.
	validPreparationInstrument, err := s.validEnumerationDataManager.GetValidPreparationInstrument(ctx, validPreparationInstrumentID)
	if errors.Is(err, sql.ErrNoRows) {
		errRes := types.NewAPIErrorResponse("not found", types.ErrDataNotFound, responseDetails)
		s.encoderDecoder.EncodeResponseWithStatus(ctx, res, errRes, http.StatusNotFound)
		return
	...
	if err = s.validEnumerationDataManager.UpdateValidPreparationInstrument(ctx, validPreparationInstrument); err != nil {
	...
	dcm := &types.DataChangeMessage{
		EventType:                  types.ValidPreparationInstrumentUpdatedServiceEventType,
		ValidPreparationInstrument: validPreparationInstrument,
		UserID:                     sessionCtxData.Requester.UserID,
	}
...
func (s *service) ArchiveValidPreparationInstrumentHandler(res http.ResponseWriter, req *http.Request) {
	...
	exists, err := s.validEnumerationDataManager.ValidPreparationInstrumentExists(ctx, validPreparationInstrumentID)
	...
	if err = s.validEnumerationDataManager.ArchiveValidPreparationInstrument(ctx, validPreparationInstrumentID); err != nil {
	...
	dcm := &types.DataChangeMessage{
		EventType: types.ValidPreparationInstrumentArchivedServiceEventType,
		UserID:    sessionCtxData.Requester.UserID,
	}
