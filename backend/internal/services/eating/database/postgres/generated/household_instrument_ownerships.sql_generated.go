...
SELECT
	...
	(
		...
		WHERE household_instrument_ownerships.archived_at IS NULL
			AND
			household_instrument_ownerships.created_at > COALESCE($1, (SELECT NOW() - '999 years'::INTERVAL))
			...
			AND (NOT COALESCE($5, false)::boolean OR household_instrument_ownerships.archived_at = NULL)
			AND household_instrument_ownerships.belongs_to_household = $6
	) AS filtered_count,
	(
		SELECT COUNT(household_instrument_ownerships.id)
		FROM household_instrument_ownerships
