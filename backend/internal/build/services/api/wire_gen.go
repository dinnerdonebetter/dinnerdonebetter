...
func Build(ctx context.Context, cfg *config.APIServiceConfig) (http.Server, error) {
	...
	workerService, err := workers.ProvideService(logger, serverEncoderDecoder, tracerProvider, worker, mealplangrocerylistinitializerWorker, mealplantaskcreatorWorker)
	if err != nil {
		return nil, err
	}
