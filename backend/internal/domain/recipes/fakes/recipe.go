...
func BuildFakeRecipe() *recipes.Recipe {
...
func BuildFakeRecipesList() *filtering.QueryFilteredResult[recipes.Recipe] {
	var examples []*recipes.Recipe
	for i := 0; i < exampleQuantity; i++ {
		examples = append(examples, BuildFakeRecipe())
	}

	return &filtering.QueryFilteredResult[recipes.Recipe]{
...
func BuildFakeRecipeUpdateRequestInput() *recipes.RecipeUpdateRequestInput {
...
func BuildFakeRecipeCreationRequestInput() *recipes.RecipeCreationRequestInput {
	...
	examplePrepTaskInput.RecipeSteps = []*recipes.RecipePrepTaskStepWithinRecipeCreationRequestInput{
		{
			BelongsToRecipeStepIndex: exampleCreationInput.Steps[0].Index,
			SatisfiesRecipeStep:      false,
		},
	}
	exampleCreationInput.PrepTasks = []*recipes.RecipePrepTaskWithinRecipeCreationRequestInput{
