...
func (q *Querier) GetRecipeStep(ctx context.Context, recipeID, recipeStepID string) (*types.RecipeStep, error) {
	...
	recipeStep := &types.RecipeStep{
		...
		Preparation: types.ValidPreparation{
			...
			MaximumInstrumentCount:      database.Uint16PointerFromNullInt32(result.ValidPreparationMaximumInstrumentCount),
			ArchivedAt:                  database.TimePointerFromNullTime(result.ValidPreparationArchivedAt),
			MaximumIngredientCount:      database.Uint16PointerFromNullInt32(result.ValidPreparationMaximumIngredientCount),
			LastUpdatedAt:               database.TimePointerFromNullTime(result.ValidPreparationLastUpdatedAt),
			MaximumVesselCount:          database.Uint16PointerFromNullInt32(result.ValidPreparationMaximumVesselCount),
			...
			MinimumIngredientCount:      uint16(result.ValidPreparationMinimumIngredientCount),
			MinimumInstrumentCount:      uint16(result.ValidPreparationMinimumInstrumentCount),
			MinimumVesselCount:          uint16(result.ValidPreparationMinimumVesselCount),
