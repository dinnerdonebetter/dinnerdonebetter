...
SELECT
	...
	(
		...
		WHERE meal_plan_option_votes.archived_at IS NULL
			...
			AND meal_plans.archived_at IS NULL
			AND meal_plans.id = $3
			AND meal_plan_option_votes.created_at > COALESCE($4, (SELECT NOW() - '999 years'::INTERVAL))
			AND meal_plan_option_votes.created_at < COALESCE($5, (SELECT NOW() + '999 years'::INTERVAL))
			AND (
				meal_plan_option_votes.last_updated_at IS NULL
				OR meal_plan_option_votes.last_updated_at > COALESCE($6, (SELECT NOW() - '999 years'::INTERVAL))
			)
			AND (
				meal_plan_option_votes.last_updated_at IS NULL
				OR meal_plan_option_votes.last_updated_at < COALESCE($7, (SELECT NOW() + '999 years'::INTERVAL))
			)
			AND (NOT COALESCE($8, false)::boolean OR meal_plan_option_votes.archived_at = NULL)
	) AS filtered_count,
	(
		SELECT COUNT(meal_plan_option_votes.id)
		FROM meal_plan_option_votes
