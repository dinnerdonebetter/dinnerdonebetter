...
import (
	...
	"github.com/dinnerdonebetter/backend/internal/repositories/postgres/recipeenums/generated"
)

var (
	_ recipeenums.ValidIngredientGroupDataManager = (*Querier)(nil)
)
...
func (q *Querier) GetValidIngredientGroup(ctx context.Context, validIngredientGroupID string) (*recipeenums.ValidIngredientGroup, error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
	...
	validIngredientGroup := &recipeenums.ValidIngredientGroup{
	...
	for _, memberResult := range membersResults {
		validIngredientGroup.Members = append(validIngredientGroup.Members, &recipeenums.ValidIngredientGroupMember{
			...
			ValidIngredient: recipeenums.ValidIngredient{
...
func (q *Querier) SearchForValidIngredientGroups(ctx context.Context, query string, filter *filtering.QueryFilter) ([]*recipeenums.ValidIngredientGroup, error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
	...
	validIngredientGroups := []*recipeenums.ValidIngredientGroup{}
	for _, result := range results {
		validIngredientGroup := &recipeenums.ValidIngredientGroup{
			...
			Slug:          result.Slug,
			Description:   result.Description,
			Members:       []*recipeenums.ValidIngredientGroupMember{},
		}
...
func (q *Querier) GetValidIngredientGroups(ctx context.Context, filter *filtering.QueryFilter) (x *filtering.QueryFilteredResult[recipeenums.ValidIngredientGroup], err error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
	...
	x = &filtering.QueryFilteredResult[recipeenums.ValidIngredientGroup]{
...
func (q *Querier) CreateValidIngredientGroup(ctx context.Context, input *recipeenums.ValidIngredientGroupDatabaseCreationInput) (*recipeenums.ValidIngredientGroup, error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
	...
	x := &recipeenums.ValidIngredientGroup{
		ID:          input.ID,
		Name:        input.Name,
		Description: input.Description,
		Slug:        input.Slug,
	...
	for i := range input.Members {
		m := input.Members[i]
		var member *recipeenums.ValidIngredientGroupMember
...
func (q *Querier) CreateValidIngredientGroupMember(ctx context.Context, db database.SQLQueryExecutorAndTransactionManager, groupID string, input *recipeenums.ValidIngredientGroupMemberDatabaseCreationInput) (*recipeenums.ValidIngredientGroupMember, error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
	...
	x := &recipeenums.ValidIngredientGroupMember{
		ID:              input.ID,
		BelongsToGroup:  groupID,
		ValidIngredient: recipeenums.ValidIngredient{ID: input.ValidIngredientID},
		CreatedAt:       q.CurrentTime(),
	}
...
func (q *Querier) UpdateValidIngredientGroup(ctx context.Context, updated *recipeenums.ValidIngredientGroup) error {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
