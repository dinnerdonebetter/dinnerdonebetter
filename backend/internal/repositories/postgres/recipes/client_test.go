...
func buildDatabaseClientForTest(t *testing.T) (*Querier, *pgcontainers.PostgresContainer) {
	...
	mealPlanningRepo := mealplanning.ProvideRepository(logging.NewNoopLogger(), tracing.NewNoopTracerProvider(), auditLogEntryRepo, identityRepo, recipeEnumsRepository, pgc)

	c := ProvideSettingsRepository(logging.NewNoopLogger(), tracing.NewNoopTracerProvider(), auditLogEntryRepo, identityRepo, mealPlanningRepo, recipeEnumsRepository, pgc)
	require.NoError(t, err)
...
func buildInertClientForTest(t *testing.T) *Querier {
	...
	c := ProvideSettingsRepository(logging.NewNoopLogger(), tracing.NewNoopTracerProvider(), nil, nil, nil, nil, &database.MockClient{})
