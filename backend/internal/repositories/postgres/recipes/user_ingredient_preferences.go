...
var (
	_ recipes.UserIngredientPreferenceDataManager = (*Querier)(nil)
)
...
func (q *Querier) GetUserIngredientPreference(ctx context.Context, userIngredientPreferenceID, userID string) (*recipes.UserIngredientPreference, error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
	...
	userIngredientPreference := &recipes.UserIngredientPreference{
...
func (q *Querier) GetUserIngredientPreferences(ctx context.Context, userID string, filter *filtering.QueryFilter) (x *filtering.QueryFilteredResult[recipes.UserIngredientPreference], err error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
	...
	x = &filtering.QueryFilteredResult[recipes.UserIngredientPreference]{
	...
	for _, result := range results {
		x.Data = append(x.Data, &recipes.UserIngredientPreference{
			CreatedAt:     result.CreatedAt,
			LastUpdatedAt: database.TimePointerFromNullTime(result.LastUpdatedAt),
			ArchivedAt:    database.TimePointerFromNullTime(result.ArchivedAt),
			ID:            result.ID,
...
func (q *Querier) CreateUserIngredientPreference(ctx context.Context, input *recipes.UserIngredientPreferenceDatabaseCreationInput) ([]*recipes.UserIngredientPreference, error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
	...
	if input.ValidIngredientGroupID != "" {
		group, err := q.recipeEnumsRepo.GetValidIngredientGroup(ctx, input.ValidIngredientGroupID)
		if err != nil {
	...
	output := []*recipes.UserIngredientPreference{}
	for _, validIngredientID := range validIngredientIDs {
		...
		x := &recipes.UserIngredientPreference{
			ID:            id,
			Rating:        input.Rating,
			Notes:         input.Notes,
			Allergy:       input.Allergy,
...
func (q *Querier) UpdateUserIngredientPreference(ctx context.Context, updated *recipes.UserIngredientPreference) error {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
