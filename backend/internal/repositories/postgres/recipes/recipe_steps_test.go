...
import (
	...
	"github.com/dinnerdonebetter/backend/internal/domain/recipes/fakes"
...
func buildRecipeStepForTestCreation(t *testing.T, ctx context.Context, recipeID string, dbc *Querier) *types.RecipeStep {
	...
	validIngredientState := recipeenumsrepo.CreateValidIngredientStateForTest(t, ctx, nil, dbc.recipeEnumsRepo)

	preparation := recipeenumsrepo.CreateValidPreparationForTest(t, ctx, nil, dbc.recipeEnumsRepo)
	ingredient := recipeenumsrepo.CreateValidIngredientForTest(t, ctx, nil, dbc.recipeEnumsRepo)
	measurementUnit1 := recipeenumsrepo.CreateValidMeasurementUnitForTest(t, ctx, nil, dbc.recipeEnumsRepo)
	...
	instrument := recipeenumsrepo.CreateValidInstrumentForTest(t, ctx, nil, dbc.recipeEnumsRepo)
	...
	measurementUnit2 := recipeenumsrepo.CreateValidMeasurementUnitForTest(t, ctx, nil, dbc.recipeEnumsRepo)
	exampleVessel := recipeenumfakes.BuildFakeValidVessel()
	exampleVessel.CapacityUnit = measurementUnit2
	vessel := recipeenumsrepo.CreateValidVesselForTest(t, ctx, exampleVessel, dbc.recipeEnumsRepo)
