...
import (
	...
	recipesfakes "github.com/dinnerdonebetter/backend/internal/domain/recipes/fakes"
...
func buildRecipeForTestCreation(t *testing.T, ctx context.Context, userID string, dbc *Querier) *recipes.Recipe {
	t.Helper()
	...
	exampleRecipe := recipesfakes.BuildFakeRecipe()

	exampleRecipeMedia := recipesfakes.BuildFakeRecipeMedia()
	exampleRecipeMedia.BelongsToRecipe = &exampleRecipe.ID
	exampleRecipe.Media = []*recipes.RecipeMedia{
		exampleRecipeMedia,
	}

	exampleRecipePrepTask := recipesfakes.BuildFakeRecipePrepTask()
	exampleRecipePrepTask.BelongsToRecipe = exampleRecipe.ID
	exampleRecipePrepTask.TaskSteps = []*recipes.RecipePrepTaskStep{
		{
			ID:                      identifiers.New(),
	...
	exampleRecipe.PrepTasks = []*recipes.RecipePrepTask{
		exampleRecipePrepTask,
	}

	exampleRecipe.Steps = []*recipes.RecipeStep{
	...
	for i := range exampleRecipe.Steps {
		exampleRecipe.Steps[i].Media = []*recipes.RecipeMedia{}
		exampleRecipe.Steps[i].CompletionConditions = nil // []*recipes.RecipeStepCompletionCondition{}
	}

	return exampleRecipe
}

func createRecipeForTest(t *testing.T, ctx context.Context, exampleRecipe *recipes.Recipe, dbc *Querier, alsoCreateMeal bool) *recipes.Recipe {
	t.Helper()
...
func TestQuerier_ArchiveRecipe(T *testing.T) {
	...
	T.Run("with invalid recipe ID", func(t *testing.T) {
		...
		exampleAccountID := recipesfakes.BuildFakeID()
...
func Test_findCreatedRecipeStepProductsForIngredients(T *testing.T) {
	...
	T.Run("sopa de frijol", func(t *testing.T) {
		...
		exampleRecipeInput := &recipes.RecipeDatabaseCreationInput{
			Name:        "sopa de frijol",
			Description: "",
			Steps: []*recipes.RecipeStepDatabaseCreationInput{
				{
					Products: []*recipes.RecipeStepProductDatabaseCreationInput{
						{
							ID:                recipesfakes.BuildFakeID(),
							Name:              productName,
							MeasurementUnitID: &recipeenumfakes.BuildFakeValidMeasurementUnit().ID,
							Type:              recipes.RecipeStepProductIngredientType,
						},
					},
					Notes:         "first step",
					...
					Products: []*recipes.RecipeStepProductDatabaseCreationInput{
						{
							ID:   recipesfakes.BuildFakeID(),
							Name: productName,
							Type: recipes.RecipeStepProductInstrumentType,
						},
					},
