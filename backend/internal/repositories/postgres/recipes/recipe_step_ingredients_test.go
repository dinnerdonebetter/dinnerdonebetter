...
func TestQuerier_Integration_RecipeStepIngredients(t *testing.T) {
	...
	validMeasurementUnit := recipeenums.CreateValidMeasurementUnitForTest(t, ctx, nil, dbc.recipeEnumsRepo)
	validIngredient := recipeenums.CreateValidIngredientForTest(t, ctx, nil, dbc.recipeEnumsRepo)
	exampleRecipeStepIngredient := fakes.BuildFakeRecipeStepIngredient()
