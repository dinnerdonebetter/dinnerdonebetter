...
func TestQuerier_Integration_AccountInstrumentOwnerships(t *testing.T) {
	...
	instrument := recipeenums.CreateValidInstrumentForTest(t, ctx, nil, dbc.recipeEnumsRepo)
	...
	for i := 0; i < exampleQuantity; i++ {
		newInstrument := recipeenums.CreateValidInstrumentForTest(t, ctx, nil, dbc.recipeEnumsRepo)
