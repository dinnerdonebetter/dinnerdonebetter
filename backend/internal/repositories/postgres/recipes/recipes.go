...
import (
	...
	generated2 "github.com/dinnerdonebetter/backend/internal/repositories/postgres/recipes/generated"
)

var (
	_ recipes.RecipeDataManager = (*Querier)(nil)
)
...
func (q *Querier) getRecipe(ctx context.Context, recipeID string) (*recipes.Recipe, error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
	...
	for _, result := range results {
		if x == nil {
			x = &recipes.Recipe{
				...
				PrepTasks:         []*recipes.RecipePrepTask{},
				SupportingRecipes: []*recipes.Recipe{},
				Media:             []*recipes.RecipeMedia{},
			}
		}

		x.Steps = append(x.Steps, &recipes.RecipeStep{
...
func (q *Querier) GetRecipes(ctx context.Context, filter *filtering.QueryFilter) (x *filtering.QueryFilteredResult[recipes.Recipe], err error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
	...
	x = &filtering.QueryFilteredResult[recipes.Recipe]{
	...
	for _, result := range results {
		x.Data = append(x.Data, &recipes.Recipe{
			CreatedAt:           result.CreatedAt,
			InspiredByRecipeID:  database.StringPointerFromNullString(result.InspiredByRecipeID),
			LastUpdatedAt:       database.TimePointerFromNullTime(result.LastUpdatedAt),
			ArchivedAt:          database.TimePointerFromNullTime(result.ArchivedAt),
...
func (q *Querier) GetRecipesCreatedByUser(ctx context.Context, userID string, filter *filtering.QueryFilter) (x *filtering.QueryFilteredResult[recipes.Recipe], err error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
...
func (q *Querier) GetRecipesWithIDs(ctx context.Context, ids []string) ([]*recipes.Recipe, error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
...
func (q *Querier) SearchForRecipes(ctx context.Context, recipeNameQuery string, filter *filtering.QueryFilter) (x *filtering.QueryFilteredResult[recipes.Recipe], err error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
...
func (q *Querier) CreateRecipe(ctx context.Context, input *recipes.RecipeDatabaseCreationInput) (*recipes.Recipe, error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
	...
	if input.AlsoCreateMeal {
		if _, err = q.mealsRepo.CreateMealInDatabase(ctx, tx, &mealplanning.MealDatabaseCreationInput{
			ID:          identifiers.New(),
			Name:        x.Name,
...
func findCreatedRecipeStepProductsForIngredients(recipe *recipes.RecipeDatabaseCreationInput) {
	for _, step := range recipe.Steps {
		for _, ingredient := range step.Ingredients {
			if ingredient.ProductOfRecipeStepIndex != nil && ingredient.ProductOfRecipeStepProductIndex != nil {
				...
				relevantProductIsIngredient := recipe.Steps[*ingredient.ProductOfRecipeStepIndex].Products[*ingredient.ProductOfRecipeStepProductIndex].Type == recipes.RecipeStepProductIngredientType
...
func findCreatedRecipeStepProductsForInstruments(recipe *recipes.RecipeDatabaseCreationInput) {
	for _, step := range recipe.Steps {
		for _, instrument := range step.Instruments {
			if instrument.ProductOfRecipeStepIndex != nil && instrument.ProductOfRecipeStepProductIndex != nil {
				...
				relevantProductIsInstrument := recipe.Steps[*instrument.ProductOfRecipeStepIndex].Products[*instrument.ProductOfRecipeStepProductIndex].Type == recipes.RecipeStepProductInstrumentType
...
func findCreatedRecipeStepProductsForVessels(recipe *recipes.RecipeDatabaseCreationInput) {
	for _, step := range recipe.Steps {
		for _, vessel := range step.Vessels {
			if vessel.ProductOfRecipeStepIndex != nil && vessel.ProductOfRecipeStepProductIndex != nil {
				...
				relevantProductIsVessel := recipe.Steps[*vessel.ProductOfRecipeStepIndex].Products[*vessel.ProductOfRecipeStepProductIndex].Type == recipes.RecipeStepProductVesselType
...
func (q *Querier) UpdateRecipe(ctx context.Context, updated *recipes.Recipe) error {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
