...
var (
	_ recipes.RecipePrepTaskDataManager = (*Querier)(nil)
)
...
func (q *Querier) GetRecipePrepTask(ctx context.Context, recipeID, recipePrepTaskID string) (x *recipes.RecipePrepTask, err error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
	...
	for _, result := range results {
		if x == nil {
			x = &recipes.RecipePrepTask{
				...
				TaskSteps:                   []*recipes.RecipePrepTaskStep{},
		...
		x.TaskSteps = append(x.TaskSteps, &recipes.RecipePrepTaskStep{
...
func (q *Querier) createRecipePrepTask(ctx context.Context, querier database.SQLQueryExecutorAndTransactionManager, input *recipes.RecipePrepTaskDatabaseCreationInput) (*recipes.RecipePrepTask, error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
...
// CreateRecipePrepTask creates a recipe prep task.
func (q *Querier) CreateRecipePrepTask(ctx context.Context, input *recipes.RecipePrepTaskDatabaseCreationInput) (*recipes.RecipePrepTask, error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
...
func (q *Querier) createRecipePrepTaskStep(ctx context.Context, querier database.SQLQueryExecutorAndTransactionManager, input *recipes.RecipePrepTaskStepDatabaseCreationInput) (*recipes.RecipePrepTaskStep, error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
...
func (q *Querier) getRecipePrepTasksForRecipe(ctx context.Context, recipeID string) ([]*recipes.RecipePrepTask, error) {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
	...
	var currentRecipePrepTask *recipes.RecipePrepTask
	for _, result := range results {
		prepTaskStep := &recipes.RecipePrepTaskStep{
			ID:                      result.TaskStepID,
			BelongsToRecipeStep:     result.TaskStepBelongsToRecipeStep,
			BelongsToRecipePrepTask: result.TaskStepBelongsToRecipePrepTask,
			SatisfiesRecipeStep:     result.TaskStepSatisfiesRecipeStep,
		...
		if currentRecipePrepTask == nil {
			currentRecipePrepTask = &recipes.RecipePrepTask{
				CreatedAt:     result.CreatedAt,
				ArchivedAt:    database.TimePointerFromNullTime(result.ArchivedAt),
				LastUpdatedAt: database.TimePointerFromNullTime(result.LastUpdatedAt),
				StorageTemperatureInCelsius: types.OptionalFloat32Range{
...
// GetRecipePrepTasksForRecipe gets a recipe prep task.
func (q *Querier) GetRecipePrepTasksForRecipe(ctx context.Context, recipeID string) (x []*recipes.RecipePrepTask, err error) {
	return q.getRecipePrepTasksForRecipe(ctx, recipeID)
}

// UpdateRecipePrepTask updates a recipe prep task.
func (q *Querier) UpdateRecipePrepTask(ctx context.Context, updated *recipes.RecipePrepTask) error {
	ctx, span := q.tracer.StartSpan(ctx)
	defer span.End()
