...
func ProvideQdrantClient(cfg *Config, logger logging.Logger, tracerProvider tracing.TracerProvider) (vectors.Searcher, error) {
	if cfg == nil {
...
func convertUpsertVectorToUpsertPoints(indexName string, data vectors.UpsertVector) (*qdrant.UpsertPoints, error) {
...
func (c *client) QueryVector(ctx context.Context, indexName string, queryVector []float32, _ map[string]any) ([]*vectors.QueryResult, error) {
	ctx, span := c.tracer.StartSpan(ctx)
	defer span.End()
