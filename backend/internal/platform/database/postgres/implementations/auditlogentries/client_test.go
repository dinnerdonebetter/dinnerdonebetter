...
func buildDatabaseClientForTest(t *testing.T) (*Querier, *postgres.PostgresContainer) {
	...
	c, err := ProvideAuditLogRepository(ctx, logging.NewNoopLogger(), tracing.NewNoopTracerProvider(), db)
	require.NoError(t, err)
...
func buildInertClientForTest(t *testing.T) *Querier {
	...
	c, err := ProvideAuditLogRepository(t.Context(), logging.NewNoopLogger(), tracing.NewNoopTracerProvider(), nil)
	require.NoError(t, err)
