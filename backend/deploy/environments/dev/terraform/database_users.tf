locals {
  ...
  email_prober_username                       = "email_prober"
...
# email_prober_username

resource "random_password" "email_prober_user_database_password" {
  length           = 64
  special          = true
  override_special = "#$*-_=+[]"
}

resource "google_sql_user" "email_prober_user" {
  name     = local.email_prober_username
  instance = google_sql_database_instance.dev.name
  password = random_password.email_prober_user_database_password.result
}
