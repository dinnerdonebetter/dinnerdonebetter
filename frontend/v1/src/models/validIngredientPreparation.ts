...
export class ValidIngredientPreparation {
  id: number;
  notes: string;
  validPreparationID: number;
  validIngredientID: number;
  createdOn: number;
  updatedOn?: number;
  archivedOn?: number;
  ...
  constructor() {
    this.id = 0;
    this.notes = "";
    this.validPreparationID = 0;
    this.validIngredientID = 0;
    this.createdOn = 0;
  }
...
export const fakeValidIngredientPreparationFactory = Factory.Sync.makeFactory<ValidIngredientPreparation> ({
  notes: Factory.Sync.each(() =>  faker.random.word()),
  validPreparationID: Factory.Sync.each(() =>  faker.random.number()),
  validIngredientID: Factory.Sync.each(() =>  faker.random.number()),
  ...defaultFactories,
});
