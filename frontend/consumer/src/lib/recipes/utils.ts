 ...
 */
export function stepElementIsProduct(
	x: RecipeStepIngredient | RecipeStepInstrument | RecipeStepVessel
): boolean {
 ...
 */
export function buildRecipeStepText(
	recipe: Recipe,
			...
			instrumentProducts.length === 0
				...
				: englishListFormatter.format(
						instrumentProducts.map(
							(x) =>
								`${((x.itemQuantity?.min ?? x.measurementQuantity?.min ?? 1) === 1 ? 'a' : 'the')} ${x.name}`
						)
					),
	...
	const output = (
		...
		`${intro}${prepName} ${ingredientList} ${vesselList} to yield ${productList}.`
	).trim();
...
export function determineVesselsForRecipes(recipes: Recipe[]): RecipeStepVessel[] {
	const allVessels = recipes.flatMap((recipe) =>
		(recipe.steps ?? []).flatMap((step) =>
			(step.vessels ?? []).filter(
				(v: RecipeStepVessel) =>
					(v.vessel && v.vessel.displayInSummaryLists) || !!v.recipeStepProductId
			)
		)
	);
				...
				uniqueVessels[id] = {
					...
					quantity: {
						...
						max: vessel.quantity?.max
							? (existing.quantity?.max ?? 0) + vessel.quantity.max
				...
				uniqueIngredients[id] = {
					...
					quantity: {
						...
						max: ing.quantity?.max
							? (existing.quantity?.max ?? 0) + ing.quantity.max
							: existing.quantity?.max
					}
				};
			} else {
 ...
 */
export function determineAllInstrumentsForRecipes(
	recipes: Recipe[]
): (RecipeStepInstrument | RecipeStepVessel)[] {
