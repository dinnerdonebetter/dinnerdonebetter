...
func (c *Client) unmarshalBody(ctx context.Context, res *http.Response, dest interface{}) error {
	...
	if err := argIsNotPointerOrNil(dest); err != nil {
		return observability.PrepareError(err, span, "nil marshal target")
	}

	bodyBytes, err := io.ReadAll(res.Body)
	if err != nil {
		return observability.PrepareError(err, span, "unmarshalling error response")
	}
	...
	if err = c.encoder.Unmarshal(ctx, bodyBytes, &dest); err != nil {
		return observability.PrepareError(err, span, "unmarshalling response body")
	}
