...
func (s *serviceSettingConfigurationsTestSuite) TestClient_GetServiceSettingConfiguration() {
	const expectedPathFormat = "/api/v1/settings/configurations/%s"

	s.Run("standard", func() {
		t := s.T()

		spec := newRequestSpec(true, http.MethodGet, "", expectedPathFormat, s.exampleServiceSettingConfiguration.ID)
		c, _ := buildTestClientWithJSONResponse(t, spec, s.exampleServiceSettingConfiguration)
		actual, err := c.GetServiceSettingConfiguration(s.ctx, s.exampleServiceSettingConfiguration.ID)
	...
	s.Run("with invalid service setting ID", func() {
		...
		actual, err := c.GetServiceSettingConfiguration(s.ctx, "")
